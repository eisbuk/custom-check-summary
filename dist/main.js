...
var getSha = (context3) => {
  ...
  switch (eventName) {
    ...
    default:
      payload = context3.payload;
      if (!((_a = payload.head_commit) == null ? void 0 : _a.id))
        throw new Error("Unsupported event, this action is only supported for: workflow_run, pull_request, push");
      return payload.head_commit.id;
  }
};
var getFiles = (dirname) => new Promise((res) => {
  import_fs.default.readdir(dirname, (err, files) => {
    if (err) {
      return res([[], err]);
    }
    const filePaths = files.reduce((acc, fName) => {
      const validFile = Boolean(fName.match(/\.(md|html|log)/));
      return !validFile ? acc : [...acc, import_path.default.join(dirname, fName)];
    }, []);
    return res([filePaths, null]);
  });
});

// src/createSummary.ts
var github = __toModule(require_github());
var import_fs2 = __toModule(require("fs"));
var createSummary_default = (_0) => __async(void 0, [_0], function* ({
  filepath,
  ...
  const fileTitle = filepath.split("/").slice(-1)[0].replace(/[-_]/g, " ").replace(/\.(md|html|log)/, "");
  const title = [titlePrefix, `"${fileTitle}"`].join(" ");
  const {
    data: { id }
  } = yield octokit.rest.checks.create(__spreadValues({
    head_sha: sha,
    name: title,
    status: "in_progress",
    output: {
  ...
  const summary = yield new Promise((res) => {
  ...
  yield octokit.rest.checks.update(__spreadValues({
    check_run_id: id,
    status: "completed",
    conclusion: "success",
    output: {
...
(() => __async(void 0, null, function* () {
  ...
  const summaryTitle = core.getInput("SUMMARY_TITLE");
  const sha = getSha(github2.context);
  const dirpath = import_path2.default.join(process.cwd(), dirname);
  console.log(`Looking for log files in ${dirname}`);
  const [filesToRead, err] = yield getFiles(dirpath);
  if (err)
    throw err;
  ...
  const createSummaryJobs = filesToRead.map((filepath, index) => createSummary_default({ filepath, token, index: index + 1, summaryTitle, sha }));
  yield Promise.all(createSummaryJobs);
}))();
/*!
